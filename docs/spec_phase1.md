...
| inferring | 推論中のセッションID一覧（TCP接続検査による判定） |
...
- **応答待ちインジケーター** — 推論中のセッションにスピナー＋「応答待ち」表示。バックエンドのTCP接続検査を権威的判定とし、ポーリングで自動補正
...
#### 推論終了の猶予期間（pendingEnd）

バックエンドが「推論なし」と1回報告しても即座に終了扱いにせず、1ポーリング周期の猶予を設ける。TCP接続検査は瞬間的な切断を拾うことがあるため、2回連続で「推論なし」を確認してから終了を確定する。

| 状態 | バックエンド報告 | 遷移先 |
|------|----------------|--------|
| 推論中（確定） | 推論なし | 猶予中（pendingEnd）— まだUIは推論中表示 |
| 猶予中（pendingEnd） | 推論なし | 終了確定 — UIから推論中表示を除去 |
| 猶予中（pendingEnd） | 推論あり | 推論中に復帰 — 猶予解除 |

#### フロント状態管理

ポーリング状態遷移は `poll_logic.mjs` に純粋関数として切り出されている。

| 状態フィールド | 説明 |
|--------------|------|
| `inferring[]` | 推論中セッションID（確定 + 猶予中を含む） |
| `pendingEnd[]` | 猶予中セッションID（バックエンドが1回「推論なし」と報告） |
| `activeStreams[]` | SSEストリーム中のセッションID（sendMessage〜完了まで） |

### 推論中判定

CLIサブプロセスが Anthropic API（port 443）への TCP 接続を持っているかを `psutil` で検査する。
...
サーバー再起動後も、PIDファイルを走査して孤児プロセスの生存・推論状態を追跡できる。

| 状態 | 処理 |
|------|------|
| PID生存 + TCP接続あり | 推論中 → `inferring` に含める |
| PID生存 + TCP接続なし | 推論終了 → kill してPIDファイル削除 |
| PID死亡 | PIDファイル削除 |
