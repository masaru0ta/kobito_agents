...
- `GET /api/agents/{id}/process-status?watching={session_id}` — 推論中セッション一覧 + 更新検知情報
...
| inferring | 推論中のセッションID一覧 |
| dir_mtime | セッションJSONL群の最大更新時刻（一覧変化の検知に使用） |
| watching_mtime | `watching` パラメータで指定したセッションのJSONL更新時刻。`watching` 未指定時は省略される |
| processes | デバッグ用プロセス詳細一覧（後述） |

**processes フィールド**

| フィールド | 説明 |
|-----------|------|
| source | `"pool"`（プール管理中）または `"pidfile"`（孤児プロセス） |
| connected | port 443へのTCP接続あり（API通信中） |
| waiting_result | メッセージ送信後〜resultイベント受信までTrue（プール内プロセスのみ） |
| jsonl_last_role | JSONLの末尾user/assistantエントリのrole（`"assistant"` / `"user"` / `null`） |
| inferring | 3シグナル統合による推論中判定（後述） |
...
|----|-----------|------|
| 901px以上 | 3カラム | 通常表示 |
| 601〜900px | 2カラム | サイドバーを `☰` ボタンでスライド開閉。サイドバー外クリックで閉じる |
| 600px以下 | 1カラム | セッション一覧のみ表示。セッション選択でチャット画面にスライド切替 |

- `☰` ボタンは中央ペインのタブ左端に表示（900px以下のみ）
- 1カラム時もサイドバーは `☰` で開閉可能
- チャットヘッダー左端の `←` ボタンでセッション一覧に戻る（600px以下のみ）
- 設定画面にも `←` ボタンで会話タブに戻れる
- `viewport-fit=cover` + `env(safe-area-inset-*)` でノッチ・ホームインジケーター対応
- ヘッダーを1行に収めるコンパクトレイアウト（600px以下）

### セッション一覧

- 日時（`created_at`, `updated_at`）・メッセージ数（件数）・タイトル・プレビュー・モデルティアを表示
- **応答待ちインジケーター** — 推論中のセッションにスピナー＋「応答待ち」表示。バックエンドの3シグナル統合判定を権威とし、ポーリングで自動補正
...
1. **推論中セッション (`inferring`)** → 応答待ちインジケーターの表示/除去を同期
2. **`dir_mtime` 変化** → セッション一覧を再取得（CLIで新規セッション作成・既存セッション更新を検知）
3. **`watching_mtime` 変化** → 表示中セッションの履歴を再取得（CLIでの書き込みをリアルタイム反映）
   - SSEストリーム中のセッションはスキップ（推論中でもSSEがなければ更新する）
   - 同一ポーリング内で推論終了による再取得が既に発生している場合はスキップ（重複排除）

#### フロント状態管理

| 状態フィールド | 説明 |
|--------------|------|
| `inferringSessions` | バックエンドから受け取った推論中セッションID（Set）。ポーリングごとに上書き |
| `activeStreams` | SSEストリーム中のセッションID（Set）。sendMessage〜SSE完了まで保持 |

- 送信時に楽観的更新: `inferringSessions` にセッションIDを追加し、即座にインジケーター表示
- ポーリングで差分検知: 前回と現在の `inferringSessions` を比較し、追加・削除されたセッションのみUIを更新
- SSEストリーム中は `loadSessionHistory` をスキップ（ストリーム中のDOMを上書きしない）
...
| waiting_result | JSONL更新済み | TCP | JSONL末尾 | 判定 |
|---|---|---|---|---|
| False | * | * | * | 完了 |
| True | No | * | * | 推論中（送信前の状態と区別できないため） |
| True | Yes | 切れ | * | 完了 |
| True | Yes | 接続中 | assistant | 完了 |
| True | Yes | 接続中 | user/None | 推論中 |

- **waiting_result**: `send_message()` でTrueにセット、`result`イベント受信でFalseにリセット。SSE切断・タイムアウト時も強制リセット
- **JSONL更新済み**: メッセージ送信時刻（`message_sent_at`）より後にJSONLが更新されているか
- **TCP**: Anthropic API（port 443）への`ESTABLISHED`接続。推論中でも切れないことがあるため、切れていれば完了確定、接続中は参考情報
- **JSONL末尾**: JSONLの末尾エントリがassistantならAIの返答が書き込まれた = 完了

#### 孤児プロセスの判定

`waiting_result`フラグを持てないため、TCP + JSONLで代替判定する。

| TCP | JSONL末尾 | 処理 |
|-----|---------|------|
| 接続中 かつ assistantでない | 推論中 → `inferring` に含める |
| それ以外 | 推論終了 → kill してPIDファイル削除 |

#### プロセス追跡（PIDファイル）

CLIサブプロセス起動時に `{project}/.kobito/alive/{session_id}.pid` にプロセスIDを記録する。
サーバー再起動後も、PIDファイルを走査して孤児プロセスを追跡できる。

#### サーバーライフサイクル

- **起動時**: PIDファイルを走査し、孤児プロセスをクリーンアップ（推論終了済みは kill）
- **シャットダウン時**: 推論中プロセスの完了を待ってから（最大60秒）全プロセスを終了

## サーバー

- FastAPI + uvicorn、ポート8200
- `start.bat` でラッパーループ起動（uvicorn終了後に自動再起動）。初回起動時のみブラウザを自動起動
- `0.0.0.0` バインドにより LAN 上の全デバイスからアクセス可能（VPN経由も含む）
- **手動再起動**: UIのサイドバー下部「再起動」ボタン → `/api/restart` → SIGTERMでuvicorn終了 → ラッパーが再起動
- ホットリロード（`--reload`）は廃止。ソースコード変更は手動再起動で反映する
...
- [ ] inferring_session_ids が推論中のセッションIDを返す（TCP接続検査）
- [ ] PIDファイルが起動時に書き込まれ、終了時に削除される
- [ ] サーバー起動時に孤児プロセスがクリーンアップされる
- [ ] サーバー停止時に推論完了を待ってから全プロセスが終了する
...
- [ ] `GET /api/agents/{id}/process-status` で推論中セッション一覧とmtimeが返る
...
- [ ] 推論中のセッションに「応答待ち」スピナーが表示される（完了まで）
